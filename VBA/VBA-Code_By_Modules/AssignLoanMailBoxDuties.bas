Attribute VB_Name = "AssignLoanMailBoxDuties"
...
SkipDay:
    ...
    Do
        ...
        For i = 1 To lmbtbl.ListRows.Count
            ...
            maxDuties = lmbtbl.DataBodyRange(i, lmbtbl.ListColumns("Max Duties").Index).Value
            currDuties = lmbtbl.DataBodyRange(i, lmbtbl.ListColumns("Duties Counter").Index).Value
            If UCase(lmbtbl.DataBodyRange(i, lmbtbl.ListColumns("Availability Type").Index).Value) <> "SPECIFIC DAYS" And currDuties < maxDuties Then
        ...
        If needsReassignment Then
            ...
            For r = lastTwoWeeksStart To LAST_ROW_ROSTER
                If wsRoster.Cells(r, LMB_COL).Value <> "" And wsRoster.Cells(r, LMB_COL).Value <> "CLOSED" Then
                    staffName = wsRoster.Cells(r, LMB_COL).Value
                    For i = 1 To lmbtbl.ListRows.Count
                        If lmbtbl.DataBodyRange(i, lmbtbl.ListColumns("Name").Index).Value = staffName And _
                           UCase(lmbtbl.DataBodyRange(i, lmbtbl.ListColumns("Availability Type").Index).Value) <> "SPECIFIC DAYS" Then
                            wsRoster.Cells(r, LMB_COL).Value = ""
            ...
            For r = lastTwoWeeksStart To LAST_ROW_ROSTER
                ...
                If wsRoster.Cells(r, LMB_COL).Value <> "" Or wsRoster.Cells(r, LMB_COL).Value = "CLOSED" Then GoTo SkipReassignDay
                
                For i = 1 To UBound(tmpStaff)
                    staffName = tmpStaff(i)
                    For j = 1 To lmbtbl.ListRows.Count
                        If lmbtbl.DataBodyRange(j, lmbtbl.ListColumns("Name").Index).Value = staffName And _
                           UCase(lmbtbl.DataBodyRange(j, lmbtbl.ListColumns("Availability Type").Index).Value) <> "SPECIFIC DAYS" Then
                            maxDuties = lmbtbl.DataBodyRange(j, lmbtbl.ListColumns("Max Duties").Index).Value
                            currDuties = lmbtbl.DataBodyRange(j, lmbtbl.ListColumns("Duties Counter").Index).Value
                        ...
                        End If
                    Next j
                    If wsRoster.Cells(r, LMB_COL).Value <> "" Then Exit For
                Next i
SkipReassignDay:
            Next r
...
Function CheckWeeklyLimit(staffName As String, rowNum As Long, startRow As Long, endRow As Long) As Boolean
    ...
    For i = weekStart To weekEnd
        If i >= startRow And i < endRow And (ws.Cells(i, LMB_COL).Value = staffName Or ws.Cells(i, SAT_AOH_COL1).Value = staffName Or ws.Cells(i, SAT_AOH_COL2).Value = staffName) Then
